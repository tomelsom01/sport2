...
<head>
    ...
    <link rel="shortcut icon" href="{{ asset('pointbreaklogo.png') }}" type="image/x-icon">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>{{ $title ?? 'Level X' }}</title>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
                                <img src="{{ asset('pointbreaklogo.png') }}" class="h-8 rounded-full" alt="Sport Logo" />
                        ...
                        <div class="flex flex-col md:flex-row md:items-center md:space-x-4 md:mx-1">
                            ...
                            @auth
                                ...
                                <form method="POST" action="{{ route('logout') }}" class="inline">
                                    @csrf
                                    <button type="submit" class="neon-pink focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-4 py-2 text-center">Logout</button>
                                </form>
                            @else
                                <a href="{{ route('register') }}" class="neon-pink focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-4 py-2 text-center">Sign Up</a>
                                <a href="{{ route('login') }}" class="neon-pink focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-4 py-2 text-center">Login</a>
                            @endauth
                        </div>
